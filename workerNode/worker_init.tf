...
# script file to run inside our machine to initialize worker inside machine
  provisioner "file" {
    source      = "../scripts/worker_init.sh"
...
# terraform worker_init file 
  provisioner "file" {
    source      = "worker_init.tf"
    destination = "/home/ubuntu/terraform/worker_init.tf"
  }
